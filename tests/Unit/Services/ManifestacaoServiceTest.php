<?php

declare(strict_types=1);

namespace PhpNfseNacional\Tests\Unit\Services;

use GuzzleHttp\Client;
use GuzzleHttp\Handler\MockHandler;
use GuzzleHttp\HandlerStack;
use GuzzleHttp\Psr7\Response;
use PhpNfseNacional\Certificate\Certificate;
use PhpNfseNacional\Certificate\Signer;
use PhpNfseNacional\Config;
use PhpNfseNacional\Dps\EventoBuilder;
use PhpNfseNacional\DTO\Endereco;
use PhpNfseNacional\DTO\MotivoRejeicao;
use PhpNfseNacional\DTO\Prestador;
use PhpNfseNacional\Enums\Ambiente;
use PhpNfseNacional\Enums\AutorManifestacao;
use PhpNfseNacional\Exceptions\SefinException;
use PhpNfseNacional\Sefin\SefinClient;
use PhpNfseNacional\Sefin\SefinEndpoints;
use PhpNfseNacional\Services\ManifestacaoService;
use PHPUnit\Framework\TestCase;

final class ManifestacaoServiceTest extends TestCase
{
    private const CHAVE = '51079092200179028000138000000000007226050575727908';
    private const CPF = '12345678909';
    // PRE + 50 dígitos chave + 6 tipoEvento (rejeição do Tomador = 203206)
    private const ID_REJ = 'PRE51079092200179028000138000000000007226050575727908203206';

    public function test_confirmar_aceita_resposta_envelope_gzip(): void
    {
        $service = $this->buildService($this->respostaOk(135));

        $resp = $service->confirmar(self::CHAVE, AutorManifestacao::Prestador);

        // parsearResposta retorna 100 implícito quando há eventoNfseXmlGZipB64
        self::assertSame(100, $resp->cStat);
    }

    public function test_confirmar_aceita_E0840_idempotente(): void
    {
        $service = $this->buildService($this->respostaErro('E0840', 'Confirmação já vinculada'));

        $resp = $service->confirmar(self::CHAVE, AutorManifestacao::Tomador);

        self::assertSame(840, $resp->cStat);
        self::assertTrue($resp->eventoIdempotente());
    }

    public function test_rejeitar_envia_motivo_e_xMotivo(): void
    {
        $service = $this->buildService($this->respostaOk(135));

        $resp = $service->rejeitar(
            self::CHAVE,
            AutorManifestacao::Tomador,
            MotivoRejeicao::Outros,
            'Cliente desistiu do serviço',
        );

        self::assertSame(100, $resp->cStat);
    }

    public function test_rejeitar_motivo_outros_sem_xMotivo_falha_validacao(): void
    {
        // Validação acontece ANTES do HTTP — passar mock vazio garante que não houve chamada
        $service = $this->buildService(null);

        $this->expectException(\PhpNfseNacional\Exceptions\ValidationException::class);
        $service->rejeitar(
            self::CHAVE,
            AutorManifestacao::Tomador,
            MotivoRejeicao::Outros,
            '',
        );
    }

    public function test_anular_rejeicao_aceita_PRE_id_e_converte(): void
    {
        $service = $this->buildService($this->respostaOk(135));

        $resp = $service->anularRejeicao(
            self::CHAVE,
            self::CPF,
            self::ID_REJ,           // formato PRE+56dig
            'Rejeição feita por engano',
        );

        self::assertSame(100, $resp->cStat);
    }

    public function test_anular_rejeicao_aceita_id_em_59_digitos_puros(): void
    {
        $service = $this->buildService($this->respostaOk(135));

        $resp = $service->anularRejeicao(
            self::CHAVE,
            self::CPF,
            '51079092200179028000138000000000007226050575727908203206001',
            'Rejeição feita por engano',
        );

        self::assertSame(100, $resp->cStat);
    }

    public function test_rejeitar_cStat_diferente_lanca_SefinException(): void
    {
        $service = $this->buildService($this->respostaErro('E1235', 'Falha no esquema XML'));

        $this->expectException(SefinException::class);
        $service->rejeitar(
            self::CHAVE,
            AutorManifestacao::Prestador,
            MotivoRejeicao::Duplicidade,
        );
    }

    private function buildService(?Response $response): ManifestacaoService
    {
        $endereco = new Endereco('Av X', '100', 'Centro', '01310100', '3550308', 'SP');
        $prestador = new Prestador('00179028000138', '11408', 'CARTORIO TESTE', $endereco);
        $config = new Config($prestador, Ambiente::Homologacao);
        $endpoints = new SefinEndpoints(Ambiente::Homologacao);

        $cert = new Certificate(
            privateKeyPem: 'DUMMY',
            certificatePem: 'DUMMY',
            validade: new \DateTimeImmutable('+1 year'),
            cnpj: '00179028000138',
            subjectCN: 'TESTE',
        );

        $http = new Client(['handler' => HandlerStack::create(
            new MockHandler($response !== null ? [$response] : []),
        )]);
        $client = new SefinClient($config, $cert, $endpoints, $http);

        $signer = new class($cert) extends Signer {
            public function sign(string $xml, string $elementName, string $idAttribute = 'Id'): string
            {
                return $xml;
            }
        };

        return new ManifestacaoService(
            builder: new EventoBuilder($config),
            signer: $signer,
            client: $client,
            endpoints: $endpoints,
        );
    }

    private function respostaOk(int $cStat): Response
    {
        $xmlInterno = '<?xml version="1.0"?><evento><cStat>' . $cStat . '</cStat></evento>';
        $body = json_encode([
            'tipoAmbiente' => 2,
            'eventoNfseXmlGZipB64' => base64_encode(gzencode($xmlInterno) ?: ''),
        ]);
        self::assertNotFalse($body);
        return new Response(200, ['Content-Type' => 'application/json'], $body);
    }

    private function respostaErro(string $codigo, string $descricao): Response
    {
        $body = json_encode([
            'tipoAmbiente' => 2,
            'erro' => [['codigo' => $codigo, 'descricao' => $descricao]],
        ]);
        self::assertNotFalse($body);
        return new Response(200, ['Content-Type' => 'application/json'], $body);
    }
}
